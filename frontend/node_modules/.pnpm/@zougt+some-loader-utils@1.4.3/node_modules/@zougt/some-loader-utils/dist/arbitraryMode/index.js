  ...
  cssColors.forEach(colorString => {
    ...
    if (!finded && defaultPrimaryColor) {
      ...
      if (mixColor) {
        sourceColorMap[colorString] = {
          mixColorString: mixColor.hex(),
          weight // varColorString: defaultPrimaryColor,

        };
      } else {
        sourceColorMap[colorString] = {
          percentGias: getHsvPercentGias(primaryVarColor, resultColor) // varColorString: defaultPrimaryColor,
...
function createSetCustomThemeFile({
  ...
  const fileContent = `
${importUtils ? 'import { getSetNewThemeMethod } from "@zougt/some-loader-utils/dist/arbitraryMode/browser";' : ''}
...
function setCustomTheme(options) {
  ...
  return _prettier.default.resolveConfig(process.cwd()).then(options => {
    const setCustomThemeConent = _prettier.default.format(`${preAppendedContent || ''}${fileContent}${appendedContent || ''}`, { ...(options || {}),
      parser: 'babel'
    });
