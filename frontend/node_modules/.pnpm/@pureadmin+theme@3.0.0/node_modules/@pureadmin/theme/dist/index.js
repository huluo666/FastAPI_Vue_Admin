...
function themePreprocessorPlugin(options = {}) {
  ...
  let defaultOptions = {
    ...
    defaultScopeName: "",
    // 强制将一些颜色值的样式作为主题样式
    includeStyleWithColors: [// {color:"#ffffff",inGradient:false}
    ],
    extract: true,
  ...
  return {
    ...
    config(conf, {
      ...
      buildCommand = command; // 在对应的预处理器配置添加 multipleScopeVars 属性

      const css = conf.css || {};
      const preprocessorOptions = css.preprocessorOptions || {};
      processorNames.forEach(lang => {
        const langOptions = options[lang] || {}; // 合并参数
        ...
        if (Array.isArray(langOptions.multipleScopeVars) && langOptions.multipleScopeVars.length) {
          preprocessorOptions[lang] = { ...(preprocessorOptions[lang] || {}),
    ...
    configResolved(resolvedConfig) {
      ...
      if (!defaultOptions.arbitraryMode) {
        ...
        const browerPreprocessorOptions = { ...defaultOptions,
          multipleScopeVars: allmultipleScopeVars
        };

        const packRoot = require.resolve(_package.default.name).replace(/[\\/]index\.js$/, "").replace(/\\/g, "/"); // 将一些参数打入到 toBrowerEnvs.js , 由brower-utils.js 获取
      ...
      if (defaultOptions.arbitraryMode && preCustomThemeOutputPath !== defaultOptions.customThemeOutputPath) {
        ...
        return (0, _createSetCustomTheme.createSetCustomTheme)({ ...defaultOptions,
          buildCommand,
    ...
    buildStart() {
      return Promise.all(processorNames.map(lang => {
        const langName = lang === "scss" ? "sass" : lang; // 得到 require('less') 时的绝对路径

        const resolved = require.resolve(langName).replace(/\\/g, "/");

        const pathnames = resolved.split("/"); // 存在类似 _less@ 开头的，兼容cnpm install

        const index = pathnames.findIndex(str => new RegExp(`^_${langName}@`).test(str) || str === langName); // 真正 less 执行的目录名称，通常情况下就是 "less" , 但cnpm install的可能就是 "_less@4.1.2@less"

        const resolveName = pathnames[index]; // 完整的 less 所在的路径
        ...
        return (0, _resetStylePreprocessor.resetStylePreprocessor)({
          ...
          // "getLess" || "getSass"
          const funName = `get${langName.slice(0, 1).toUpperCase() + langName.slice(1)}`; // 在substitute生成替代包
          ...
          _fsExtra.default.copySync(`${substituteDir}/preprocessor-substitute-options.js`, `${substitutePreprocessorDir}/preprocessor-substitute-options.js`); // require('less')时的文件名，如 "index.js"


          const mainFile = resolved.replace(resolveDir, "").replace(/^\/+/g, ""); // 向 "index.js" 中写上如 "getLess" 的调用
    ...
    transformIndexHtml(html) {
      ...
      if (arbitraryMode) {
        ...
        const themeResult = buildCommand !== "build" ? (0, _createSetCustomTheme.createSetCustomTheme)({ ...defaultOptions,
          buildCommand,
          cacheThemeStyleContent
...
function themePreprocessorHmrPlugin() {
  let parentApi = null;
  let cacheThemeStyleContent = "";
  let buildCommand = ""; // 触发热更新时的 样式文件

  const hotUpdateStyleFiles = new Set(); // 进入transform的样式文件
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (defaultOptions.arbitraryMode && /\.(less|scss|sass)(\?.+)?/.test(id)) {
        transformStyleFiles.add(id); // 当transform的的样式文件数量 到达 触发热更新的样式文件数量时，就获取主题css，并触发热更新事件 import.meta.hot.on('custom-theme-update',()=>{}）

        if (hotUpdateStyleFiles.size && hotUpdateStyleFiles.size === transformStyleFiles.size) {
          (0, _someLoaderUtils.getThemeStyleContent)();
          (0, _createSetCustomTheme.createSetCustomTheme)({ ...defaultOptions,
            buildCommand,
            cacheThemeStyleContent
          }).then(result => {
            if (result) {
